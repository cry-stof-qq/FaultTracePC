...
|---|---|---|
| **MSI** | Installation durable, déploiement par GPO | `msiexec /i FaultTracePC-1.4.0.msi` (ou double-clic) |
...
`msiexec /i FaultTracePC-1.4.0.msi FTPCLANG=en /qn`. C'est un défaut — le choix
...
powershell -ExecutionPolicy Bypass -File installer\build-msi.ps1 -Version 1.4.0
```
